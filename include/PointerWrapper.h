...
public:
    ...
    T& operator*() const {
        if (!ptr) {
            throw std::runtime_error("PointerWrapper: Null pointer dereference in operator*");
        }
        return *ptr;
    }
    ...
    T* operator->() const {
        if (!ptr) {
            throw std::runtime_error("PointerWrapper: Null pointer access in operator->");
        }
        return ptr;
    }
    ...
    T* get() const {
        if (!ptr) {
            throw std::runtime_error("PointerWrapper: Null pointer access in get()");
        }
        return ptr;
    }
